...
class BrainDQN:
    ...
    def __init__(self, args, num_node_feat, num_edge_feat):
        ...
        self.model = GATModel(in_dim, hidden_dim, out_dim, num_heads).to(self.device)
        self.target_model = GATModel(in_dim, hidden_dim, out_dim, num_heads).to(self.device)
    ...
    def train(self, x, y):
        ...
        output = self.model(action_graphs, action_nodeIDs)
        max_value, _ = torch.max(output, dim=0)
        # Calculate loss (mean squared error between max value and reward)
        # print("++++++++device++++++")
        # print(device)
        max_value = max_value.to(self.device)
        loss = F.mse_loss(max_value, torch.tensor([y], dtype=torch.float).to(self.device))
    ...
    #         features = graph.ndata['n_feat'].to(self.device)
    ...
    def predict(self, action_graphs, action_nodeIDs, target=False):
        ...
        with torch.no_grad():
            ...
            # print(action_nodeIDs)
            predictions = model(action_graphs, action_nodeIDs)
