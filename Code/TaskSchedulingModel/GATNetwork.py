...
class GATLayer(nn.Module):
    """
    Definition of a GAT message passing layer with features only on the nodes
    """

    def __init__(self, v_in_dim, v_out_dim):
        """
        ...
        self.W_a = nn.Parameter(nn.init.xavier_normal_(torch.zeros((2 * v_in_dim, v_out_dim)),
                                                       gain=SQRT_TWO))
        self.W_T = nn.Parameter(nn.init.xavier_normal_(torch.zeros((2 * v_in_dim, v_out_dim)),
                                                       gain=SQRT_TWO))
    ...
    def forward(self, g):
        """
        Forward pass of the node-only GAT layer. Note that the graph features are updated.
        :param g: the graph
...
class GATNetwork(nn.Module):
    ...
    def __init__(self, layer_features, n_hidden_layer, latent_dim, output_dim):
        ...
        :param layer_features: list [n_feat_1, n_feat_2, ...] of the dimension of the GAT embedding
    ...
    def forward(self, g, graph_pooling):
        ...
        an output on the global graph. Otherwise, output is node-dependent
